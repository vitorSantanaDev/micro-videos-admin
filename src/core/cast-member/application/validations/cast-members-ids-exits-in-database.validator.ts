import { CastMemberInMemoryRepository } from '@core/cast-member/infra/database/in-memory/cast-member-in-memory.repository'
...
describe('CastMembersIdExistsInDatabaseValidator Unit Tests', () => {
  ...
  it('should return a list of categories id', async () => {
    ...
    expect(castMembersId[0]).toMatchObject(castMember1.cast_member_id)
    expect(castMembersId[1]).toMatchObject(castMember2.cast_member_id)
  })
})
